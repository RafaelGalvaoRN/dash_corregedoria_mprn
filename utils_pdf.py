...
def gerador_relatorio_pdf(dfs, df_names, promotoria="teste", filename="relatorio.pdf",
    ...
    for df, df_name in zip(dfs, df_names):
        ...
        if any(name in df_name.lower() for name in ["extra sem impulsionamento", "relatório extraj"]):
           passivo = qtd_acervo_extra
        else:
            passivo = qtd_acervo_jud


        qtd_indices = len(df.index)
        percentual_atraso = calcula_indice_percentual_atrado(passivo, qtd_indices)
        ...
        paragraph = Paragraph("TABELA DE PERCENTUAL DE ÍNDICE DE ATRASO", styles['Heading2'])
        paragraph.hAlign = 'LEFT'
        elements.append(paragraph)

        data =  [['Acervo Total', 'Qtd de Passivo', 'Percentual Atraso %'],
                 [passivo, qtd_indices, percentual_atraso]  # Adicione seus dados reais aqui
                 ]
